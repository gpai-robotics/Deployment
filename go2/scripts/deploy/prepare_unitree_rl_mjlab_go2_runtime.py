...
REPO_ROOT = Path(os.environ["REPO"]).resolve()
VALIDATE_FSM_RUNTIME = REPO_ROOT / "go2" / "scripts" / "deploy" / "validate_unitree_mjlab_go2_fsm_runtime.py"
DEFAULT_BUNDLE = (
    REPO_ROOT
    / "go2"
    / "policies"
...
def main() -> int:
    ...
    if not args.skip_validate:
        validate_cmd = [
            ...
            bundle_policy_name,
            "--json-out",
            str(REPO_ROOT / "go2" / "artifacts" / "deployment_validation" / runtime_name / "unitree_mjlab_fsm_runtime_audit.json"),
        ]
