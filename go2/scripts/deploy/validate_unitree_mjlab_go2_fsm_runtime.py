...
REPO_ROOT = Path(
    os.environ.get(
        "REPO",
        Path(__file__).resolve().parents[1]   # fallback if REPO isn't set
    )
)
DEFAULT_GO2_DEPLOY_DIR = REPO_ROOT / "go2" / "reference_repos" / "Unitree_mjlab_repo" / "unitree_rl_mjlab" / "deploy" / "robots" / "go2"
